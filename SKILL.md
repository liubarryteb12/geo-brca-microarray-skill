...
- **`impute.knn` 必须先设种子，否则整条流水线不可复现。** 它内部用 `sample()`
  处理并列近邻；不固定种子时插补值每次运行都略有不同，下游 t 统计量 / GSEA 排序 /
  富集 p 值全部跟着变。实测未设种子时连续两轮 CI 的 GSEA 显著 GO 条目数是
  **1059 和 1110** —— 看着像蒙特卡洛波动，根因其实在插补。
  注意 `gseGO(seed=123)` 管不住这个：它确实生效了（日志无退回警告），
  但上游输入已经分叉。种子要紧挨着随机调用设置，`config.yml` 的 `analysis.seed` 不可删。
