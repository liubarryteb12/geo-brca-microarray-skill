---
name: geo-normal-pipeline-skill
description: Run an end-to-end GEO gene-expression microarray data-mining pipeline - download and clean, QC, PCA, sample correlation, limma differential expression, clustered heatmap, GO/KEGG enrichment, preranked GSEA, STRING PPI, WGCNA co-expression modules, LASSO-Cox prognostic signature, and transcription-factor regulon analysis (DoRothEA) - and ship it as a GitHub Actions workflow that uploads results as an artifact. The pipeline is config-driven and disease-agnostic: nothing in scripts/ is specific to any disease or platform, so any human or mouse microarray series works once you add assets/config.<GSE>.yml (validated on breast cancer - GSE64790 n=6 and GSE42568 n=121 - but the name and code carry no disease assumption). Use when the user asks for GEO data mining, a GEO/GSE microarray analysis, an expression analysis of any disease cohort, a small-sample limma DEG workflow, GO/KEGG enrichment on GEO data, TF/regulon or transcription-factor activity analysis on bulk expression, WGCNA or LASSO-Cox on GEO data, a GitHub Actions bioinformatics pipeline, or asks to reproduce or extend GSE64790.
license: MIT
