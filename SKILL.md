...
- **网络图按节点筛是反效果，密度要按边筛。** 390 节点 / 3284 边全画是毛线球；
...
- **颜色判据是量化的：色相相差 15° 以内视为同一个颜色。** 按这条算，
  本仓库曾经的配色有三个凭眼睛看不出来的问题：p 值直方图的 `firebrick` 与 up 红
  只差 **0.4°**（就是同一个颜色）；分类色板的棕在 protanopia 下与 up 红距离 **0.002**
  （几乎重合）；magma 序列色中段与 up 红只差 **2.9°**（序列色中段就是"上调红"）。
  改色值前跑 `node tools/check_palette.mjs`，不要在脚本里就地写字面量。
...
- **火山图"没有差异基因"通常不是倍数阈值的问题。** 判据是
  ...
  这时应该加一条**明确标注"名义 P、非 FDR 显著"**的层级让图有信息量，
  而不是放宽 FDR 或假装名义显著就是显著。纵轴统一用 raw P：
  混用 raw P 和 adj.P 会让名义显著的点画在 FDR 线之上，看着像通过了 FDR。
