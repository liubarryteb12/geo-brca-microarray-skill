...
make_dotplot <- function(x, cfg, title) {
  ...
  p <- enrichplot::dotplot(x, showCategory = n) +
    ...
    theme(axis.text.y = element_text(size = 7),
          legend.position = c(0.98, 0.98), legend.justification = c(1, 1), legend.direction = "vertical", legend.text = ggplot2::element_text(size = 7), legend.key.size = ggplot2::unit(0.6, "lines"))
  p
}
